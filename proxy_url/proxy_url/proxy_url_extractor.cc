...
namespace qh
{
    ...
    void ProxyURLExtractor::Extract( const KeyItems& keys, const std::string& raw_url, std::string& sub_url )
    {
        ...
        while (pos != std::string::npos) {
            ...
            pos = raw_url.find('&', pos=lastPos);
            std::string keyValue;
            if (pos == std::string::npos)
                keyValue = raw_url.substr(lastPos);
            else
                keyValue = raw_url.substr(lastPos, pos-lastPos);
            size_t keyPos= keyValue.find('=');
            if (keyPos != std::string::npos) {
                std::string key = keyValue.substr(0, keyPos);
                if (keys.find(key) != keys.end()) {
                    sub_url = keyValue.substr(keyPos+1);
                    break;
                }
            }
        }
